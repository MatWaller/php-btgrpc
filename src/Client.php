...
class Client
{
    ...
    public function request($method, ...$params)
    {
        try {
            ...
            if ($response->hasError()) {
                ...
                throw new Exceptions\GolddException($response->error());
            }
    ...
    /**
     * Makes async request to Gold Core.
     *
     * @param string        $method
     * @param mixed         $params
    ...
    protected function defaultConfig(array $config = [])
    {
        $defaults = [
            ...
            'port'       => 19998,
            'user'       => 'goldcashrpc',
            'password'   => '5pdjzTZrjsuJk6ivFSD65phgWZXQ5aCtXB4PWrvaFAUa',
        ];

        // use same var name as laravel-goldrpc
        if (
    ...
    protected function getHandler()
    {
        ...
        $stack->push(
            Middleware::mapResponse(function (ResponseInterface $response) {
                return GolddResponse::createFrom($response);
            }),
            'json_response'
        );
    ...
    protected function onSuccess(ResponseInterface $response, callable $callback = null)
    {
        if (!is_null($callback)) {
            if ($response->hasError()) {
                $response = new Exceptions\GolddException($response->error());
            }
    ...
    protected function handleException($exception)
    {
        if ($exception->hasResponse()) {
            ...
            if ($response->hasError()) {
                return new Exceptions\GolddException($response->error());
            }
        }
